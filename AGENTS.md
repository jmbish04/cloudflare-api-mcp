...
├── migrations/                        # D1 schema (wrangler d1 migrations apply CICD_DB --remote)
...
- **D1:** `CICD_DB` (`cloudflare-api-mcp-cicd`) — pause/resume state, leases, audit, patterns. Schema in `migrations/`; apply with `wrangler d1 migrations apply CICD_DB --remote -c ./wrangler.jsonc`.
