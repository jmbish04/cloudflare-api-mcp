...
pnpm run db:migrate     # wrangler d1 migrations apply CICD_DB --remote
pnpm run db:explain     # confirm no hot query lost its index
```

Migrations are generated by drizzle-kit from `src/db/schema.ts` (`pnpm run
db:generate`) into the same flat `migrations/*.sql` layout wrangler discovers by
default — no `migrations_pattern` needed. Never hand-write one.
